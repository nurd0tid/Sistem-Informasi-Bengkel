...
class Report extends CI_Controller
{
    ...
    public function book()
    {
        ...
        $push = [
            "pageTitle" => "Laporan Pembelian & Service",
            "dataAdmin" => $this->dataAdmin
        ];

        $this->load->view('header', $push);
        $this->load->view('book_report', $push);
        $this->load->view('footer', $push);
    }

    public function json_book($start = 0, $end = 0)
    {
        ...
        $this->datatables->setOrdering(["id", "date", "name", "type"]);
    ...
    function json($type = "sparepart", $start = 0, $end = 0)
    {
        ...
        if ($type == "sparepart") {
            ...
            $this->datatables->setOrdering(["transaction_id", "date", "name", "items", "total"]);
        } elseif ($type == "service") {
            $this->datatables->setColumn([
                '<index>',
                '<get-customer>',
            ...
            $this->datatables->setOrdering(["transaction_id", "customer",  "plat", "name", "date", "total"]);
        }
        $this->datatables->setSearchField("date");
        $this->datatables->generate();
    }
    ...
    function book_pdf($start = 0, $end = 0)
    {
        $query = $this->book_model->get($start, $end);
        ...
        if ($query->num_rows() > 0) {
            ...
            $title = "Laporan_Tutup_Buku";

            $this->load->library("pdf");

            $this->pdf->setPaper('A4', 'portrait');
            $this->pdf->filename = $title;
            $this->pdf->load_view('book_pdf', $push);
        }
    }
}
